%% -------------------------------------------------------------------
%%
%% sweeper_callback_mutate: sweep participant for sweeper testing
%%
...
-module(sweeper_callback_mutate).
...
visit_object_function({{_Bucket, _Key}, RObj}, wait_for_count, _Opts = []) ->
...
visit_object_function({{_Bucket, _Key}, RObj}, {mutate, Count, Visited}, _Opts = [])
  when Visited < Count ->
    Acc1 = {mutate, Count, Visited+1},
    {mutated, RObj, Acc1};

visit_object_function({{_Bucket, _Key}, _RObj}, Acc = {mutate, _Count, _Visited}, _Opts = []) ->
    {ok, Acc}.
