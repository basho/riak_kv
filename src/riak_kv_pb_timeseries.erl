...
-spec decode_query_permissions(ts_query_types()) ->
                                      {string(), binary()}.
decode_query_permissions({#ddl_v1{table = NewBucketType}, _}) ->
...
process({DDL = #ddl_v1{}, WithProps}, State) ->
    %% the only one that doesn't require an activated table
    create_table(DDL, WithProps, State);
...
-spec create_table(#ddl_v1{}, TableProperties::proplists:proplist(), #state{}) ->
                          {reply, #tsqueryresp{} | #rpberrorresp{}, #state{}}.
create_table(DDL = #ddl_v1{table = Table}, UserProps, State) ->
    {ok, Props1} = riak_kv_ts_util:apply_timeseries_bucket_props(DDL, UserProps),
