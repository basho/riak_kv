...
%% If a key is tagged with `colocated' then use the c prefix, this
%% groups the objects when leveldb sorts the keys. On a range scan
%% it means less skipping over non-timeseries keys.
to_object_key(Bucket, {colocated, Key}) ->
    sext:encode({c, Bucket, Key});
