%% -------------------------------------------------------------------
%%
%% riak_kv_multi_prefix_backend: switching between multiple storage engines
%%
...
-module (riak_kv_multi_prefix_backend).
...
-define(CAPABILITIES, [async_fold, iterator_refresh]).
...
backend_fold_fun(ModFun, FoldFun, Opts, AsyncFold) ->
    fun({_, Module, SubState}, {Acc, WorkList}) ->
            ...
            backend_fold_fun(Module, ModFun, SubState, FoldFun, Opts, {Acc, WorkList}, DoAsync)
    end.
